...
from .schemas import PatientBase, PatientResponse
...
def update_patient(patient_id: int, patient_update: PatientBase, db: Session = Depends(get_db)):
    db_patient = db.query(Patient).filter(Patient.id == patient_id).first()
    if db_patient is None:
        raise HTTPException(status_code=404, detail="Patient not found")
...
def search_patients(
    ...
    if first:
        query = query.filter(Patient.first.ilike(f"%{first}%"))
    if last:
        query = query.filter(Patient.last.ilike(f"%{last}%"))
    if gender:
